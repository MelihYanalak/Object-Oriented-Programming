...
namespace Melih{
	...
	Rectangle::Rectangle(double conWidth,double conLength,double conRDX,double conRDY){
		if(conWidth <= 0 || conLength <= 0){
			throw invalid_argument("");
		}
		setWidth(conWidth);    // constructor with parameter
	...
	Rectangle& Rectangle::operator++(int ignoreMe){
		...
		tempRDX = getRiDownX();
		tempRDY = getRiDownY();
		Rectangle* newRec = new Rectangle(getWidth(),getLength(),tempRDX,tempRDY);
		setRiDownX(riDownX+1.0);
		setRiDownY(riDownY+1.0);
		return *newRec;
	...
	Rectangle& Rectangle::operator+(double num)const{
		Rectangle* newRec = new Rectangle(getWidth()+num,getLength()+num,getRiDownX(),getRiDownY());
		return *newRec;
