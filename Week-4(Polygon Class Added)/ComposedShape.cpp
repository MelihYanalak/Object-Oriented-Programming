			...
			if(countA >= countB){
				...
				while((downBorder-currY)>= 0){
					while((rightBorder-currX)>=(0)){					
						...
						Polygon tempPol(tempR);
						innerVec.push_back(tempPol);
						currX = currX + innerR.getWidth();
...
inline const Polygon& ComposedShape::operator[](int index)const{
