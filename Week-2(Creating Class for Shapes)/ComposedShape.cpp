...
using namespace std;
	...
	ComposedShape::ComposedShape(Triangle bigT,Triangle smallT){
	 ...
	 vector<Rectangle> ComposedShape::getVectorR(){
		return vecRec;                         
	}
	 vector<Circle> ComposedShape::getVectorC(){        // getter functions returns an vector of class
		return vecCirc;
	}
	 vector<Triangle> ComposedShape::getVectorT(){            
		return vecTri;
	}
	 Triangle ComposedShape::getVecTri(int index)const{
		return vecTri[index];
	}
	
	 Rectangle ComposedShape::getVecRec(int index)const{       // getter functions returns an element of vector of class
		return vecRec[index];
	}
	 Circle ComposedShape::getVecCirc(int index)const{
		return vecCirc[index];
	}
			...
			if(countA >= countB){
				...
				while((downBorder-currY)>= 0){
					while((rightBorder-currX)>=(0)){
						...
						tempR.setRiDownY(currY);
						vecRec.push_back(tempR);   // then put tempR in vecRec
						currX = currX + innerR.getWidth();
			...
			if(countA >= countB){
				...
				while((downBorder-currY)>=((innerC.getRadius())*SQR/2)){
					...
					while((rightBorder-currX)>= (innerC.getRadius())){
						...
						vecCirc.push_back(tempC);  // pushback to vecCirc
						currX = currX + (2*innerC.getRadius());
					}
					control++;
					currY = currY + ((innerC.getRadius())*SQR);
				...
				*/
				...
				while((downBorder-currY)>=((innerC.getRadius())*SQR/2)){
					...
					while((rightBorder-currX)>= (innerC.getRadius())){
						//drawCirc(innerC.getRadius(),currX,currY,"green");
						tempC.setRadius(innerC.getRadius());
						tempC.setOriginX(currX);
						tempC.setOriginY(currY);
						vecCirc.push_back(tempC);
						currX = currX + (2*innerC.getRadius());
					}
					currY = currY + ((innerC.getRadius())*2);
				}
