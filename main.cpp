...
ostream& operator<<(ostream& fp,const Shape& shape){
	shape.draw(fp,"yellow");
	return fp;
	
}
void printAll(ostream& fp,vector<Shape*> vec){
	if(vec.size() <= 0){
...
void printPoly(ostream& fp,vector<Shape*> vec){
	if(vec.size() <= 0){
		throw invalid_argument("");
	}	
	int i = 0;
...
vector<Shape*> convertAll(vector<Shape*> vec) {
	if(vec.size() <= 0){
		throw invalid_argument("");
	}
	vector<Shape*> newVec;
...
vector<Shape*> sortShapes(vector<Shape*> vecP){
	if(vecP.size() <= 0){
		throw invalid_argument("");
	}	
	int i,j;
...
int main(){
	...
	try{
		...
		vec.push_back(&c2);
		vec.push_back(&t1);
		fp.open(".\\outputs\\ComposedShape.svg");
		fp<<"<svg>"<<endl;
