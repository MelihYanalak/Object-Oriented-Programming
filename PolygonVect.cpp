...
PolygonVect::PolygonVect(vector<double> ipointX,vector<double> ipointY,double ivArea,double ivPerimeter){
	pointX = ipointX;
	pointY = ipointY;
	vArea = ivArea;
	vPerimeter = ivPerimeter;
...
PolygonVect::PolygonVect(Circle& c1){
	...
	PolygonVect& PolygonVect::operator++(int ignoreMe){
		PolygonVect* temp = new PolygonVect(pointX,pointY, vArea,vPerimeter);
		for(int i = 0;i<pointX.size();i++){
			pointX[i]++;
			pointY[i]++;
		}
