...
PolygonDyn::PolygonDyn(double ivArea, double ivPerimeter, double* ipointX, double* ipointY,int isize){
	vArea = ivArea;
...
PolygonDyn::PolygonDyn(Circle& c1){
	...
	PolygonDyn& PolygonDyn::operator++(int ignoreMe){
		PolygonDyn* temp = new PolygonDyn(vArea,  vPerimeter,  pointX,  pointY,size);
		for(int i = 0;i<size;i++){
			pointX[i]++;
			pointY[i]++;
		}
